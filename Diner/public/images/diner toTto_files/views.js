...
var rmDishFrMenuView = Backbone.View.extend({
	el: 'ul#alreadyOnMenu', 
	template:_.template($('#takeDishOffMenuForm').html()),

	render:function(){
		var thisView = this; 
		this.collection.each(function(dishOut){
			thisView.$el.prepend(thisView.template({dishOut:dish.toJSON()}));
			$("#dishOn"+dishOut.id).draggable();
		});
		return this;
	}
});
